/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QueriesWhereInput } from "./QueriesWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { QueriesOrderByInput } from "./QueriesOrderByInput";

@ArgsType()
class QueriesFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => QueriesWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => QueriesWhereInput, { nullable: true })
  @Type(() => QueriesWhereInput)
  where?: QueriesWhereInput;

  @ApiProperty({
    required: false,
    type: [QueriesOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [QueriesOrderByInput], { nullable: true })
  @Type(() => QueriesOrderByInput)
  orderBy?: Array<QueriesOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { QueriesFindManyArgs as QueriesFindManyArgs };
